...
bot.hears('دارت پرتاب کن و جایزه بگیر 🎯', async (ctx) => {
  ...
  const latestRecordQuery = await knex('promo_codes').select('codes', 'active', 'created_at').where('telegram_id', ctx.chat.id + '').orderBy('date', 'desc').first();
  ...
  setTimeout(async () => {
    ...
    if (drotic.dice.value == 6 && !(hoursPassed >= 24 )) {
      ...
      await knex('promo_codes').where({ codes: getPromo[0].codes, created_at: DateTime.now() }).update({ active: 1, telegram_id: ctx.chat.id + '' });
      await ctx.reply(`
