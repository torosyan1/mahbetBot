...
کاربر عزیز 24 ساعت از پرتاب دارت شما در بازی گذشته است و الان دوباره میتونید وارد ربات بشید و دارت بازی کنید و شانس خودتون را امتحان کنید.
...
bot.hears('دارت پرتاب کن و جایزه بگیر 🎯', async (ctx) => {
  ...
  console.log(!(isUsed || !(hoursPassed >= 24)), 'aaaa', hoursPassed, hoursPassed)
  if (!(isUsed || !(hoursPassed >= 24))) {
    ...
    setTimeout(async () => {
      ...
      if (drotic.dice.value == 6 && hoursPassed >= 24 ) {
        ...
        await ctx.reply(`تبریک 😎... تبریک😎 ... شما برنده 10 هزار تومان شرط رایگان شده اید. 
  اگر در سایت ماه بت ثبت نام کرده اید لطفا" وارد سایت شوید و کد هدیه زیر را در حساب کاربری خود وارد کنید و اگر هنوز ثبت نام نکرده اید لطفا از طریق دکمه زیر ثبت نام کنید و وارد حساب کاربری خود شوید و کد هدیه را فعال کنید و لذت ببرید. 
